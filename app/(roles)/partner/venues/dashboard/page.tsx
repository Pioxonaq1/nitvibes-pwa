...
export default function VenueDashboard() {
  ...
  // Normalizamos el nombre para evitar errores de TypeScript [cite: 2025-12-25]
  const venueName = (user as any).name || user.nombre || "Sala Apolo";
  const hasFlash = (user as any).hasFlash || false;
  const venueVibe = (user as any).vibe || "Poble Sec";
      ...
      <div className="w-full max-w-4xl space-y-6 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-zinc-900/50 border border-white/5 p-4 rounded-2xl flex flex-col items-center gap-1">
            ...
            <p className="text-lg font-black leading-none">{hasFlash ? "1" : "0"}</p>
        ...
        <div className="bg-zinc-900/50 border border-white/5 rounded-[2.5rem] overflow-hidden h-64 relative group">
          ...
          <div className="absolute bottom-4 left-6 flex items-center gap-2 bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
            ...
            <span className="text-[10px] font-black uppercase italic">Ubicación: {venueVibe}</span>
          </div>
        </div>
      </div>
    </div>
